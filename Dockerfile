...
# go
RUN curl -Lo /tmp/go.tar.gz https://dl.google.com/go/go1.16.7.linux-amd64.tar.gz \
...
# aws v2
RUN curl "https://awscli.amazonaws.com/awscli-exe-linux-x86_64-2.2.29.zip" -o "/tmp/awscliv2.zip" \
 && cd /tmp \
 && unzip awscliv2.zip \
 && ./aws/install \
